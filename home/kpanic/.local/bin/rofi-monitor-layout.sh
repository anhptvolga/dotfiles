...
do
    ...
    do
        ...
        then
            ...
            COMMANDS[$index]="xrandr --output ${MONITORS[$entry_a]} --primary --auto \
                              --output ${MONITORS[$entry_b]} --auto --right-of ${MONITORS[$entry_a]}"
            index+=1
            TILES[$index]="Dual Screen ${MONITORS[$entry_a]} v ${MONITORS[$entry_b]}"
            COMMANDS[$index]="xrandr --output ${MONITORS[$entry_a]} --primary --auto \
                              --output ${MONITORS[$entry_b]} --auto --below ${MONITORS[$entry_a]}"
            index+=1
        fi
    done
done
