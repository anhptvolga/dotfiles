...
local net = lain.widget.net({
    settings = function()
        local recv = string.sub(string.format("%.3f", net_now.received / 8192), 1)
        local send = string.sub(string.format("%.3f", net_now.sent / 8192), 1)
        widget:set_markup(markup.fontfg(theme.font, "#FEFEFE", " " .. recv .. " ↓↑ " .. send .. " "))
    end
})
...
function theme.at_screen_connect(s)
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            ...
            pl(volume.widget, "#8DAA9A"),
            pl(mytextclock, "#777E76"),
            pl(wibox.widget { baticon, bat.widget, layout = wibox.layout.align.horizontal }, "#343434"),
            s.mylayoutbox,
        },
    }
end
